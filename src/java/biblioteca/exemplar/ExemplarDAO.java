...
public class ExemplarDAO extends DAO {
  ...
  public boolean alterar(Exemplar exemplar) {
	try {
            ...
            PreparedStatement pstmt = conexao.prepareStatement("Update exemplar SET status = ? WHERE numero = ? AND publicacao_isbn= ? ");
            pstmt.setString(1, exemplar.getStatus());
            pstmt.setInt(2, exemplar.getNumero());
            pstmt.setInt(3, exemplar.getPublicacao_isbn());
            pstmt.execute();
            pstmt.close();
            conexao.close();
            return true;
    ...
    public boolean inserir(Exemplar exemplar) {
        try {
            ...
            pstm.setInt(2,exemplar.getPublicacao_isbn());
    ...
    public List<Exemplar> listar() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                exemplar.setPublicacao_isbn(rs.getInt("publicacao_isbn"));
